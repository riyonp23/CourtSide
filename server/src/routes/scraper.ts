...
export async function scraperRoutes(server: FastifyInstance) {
  server.post<{ Reply: { status: string } | ErrorResponse }>(
    "/scrape",
          ...
          for (const line of lines) {
            ...
            if (eventMatch?.[1] && eventMatch[2]) {
              try {
                ...
                const eventData = JSON.parse(eventMatch[2]) as Record<string, unknown>;
  ...
  server.get<{ Reply: { lastScrapedAt: Date | null; totalTeams: number; totalPlayers: number; scraping: boolean } | ErrorResponse }>(
    "/scrape/status",
    async (_request, reply) => {
      try {
