...
interface TeamProfile {
  ...
  losses: number;
  winPct: number;
  avgKillsPerSet: number;
  avgHittingPct: number;
  avgAcesPerSet: number;
  avgDigsPerSet: number;
  avgBlocksPerSet: number;
  avgAssistsPerSet: number;
...
let profileCache: Map<string, TeamProfile> | null = null;
...
async function buildCache(): Promise<Map<string, TeamProfile>> {
...
async function getCache(): Promise<Map<string, TeamProfile>> {
  if (!profileCache) {
...
export async function predictMatchup(teamAId: string, teamBId: string): Promise<Prediction> {
  const cache = await getCache();
  const a = cache.get(teamAId);
  const b = cache.get(teamBId);
  if (!a || !b) throw new Error("Team not found in cache");
  ...
  const confidence: "high" | "medium" | "low" =
    maxProb > 0.7 ? "high" : maxProb > 0.6 ? "medium" : "low";
...
function generateKeyFactors(a: TeamProfile, b: TeamProfile, probA: number): string[] {
  ...
  if (Math.abs(a.avgHittingPct - b.avgHittingPct) > 0.01) {
    ...
    factors.push(
      `${better.name}'s hitting percentage (.${Math.round(better.avgHittingPct * 1000)}) significantly outpaces ${worse.name}'s (.${Math.round(worse.avgHittingPct * 1000)})`
    );
  }
...
export async function generateFeaturedMatchups(): Promise<Prediction[]> {
  const cache = await getCache();
  ...
  const matchupPairs: [string, string][] = [];
  const r0 = ranked[0] as TeamProfile | undefined;
  const r1 = ranked[1] as TeamProfile | undefined;
  const r2 = ranked[2] as TeamProfile | undefined;
  const r3 = ranked[3] as TeamProfile | undefined;
  ...
  const mpsf0 = mpsf[0] as TeamProfile | undefined;
  const mpsf1 = mpsf[1] as TeamProfile | undefined;
  if (mpsf0 && mpsf1) {
    const pair: [string, string] = [mpsf0.id, mpsf1.id];
    const isDupe = matchupPairs.some(
      (p) => (p[0] === pair[0] && p[1] === pair[1]) || (p[0] === pair[1] && p[1] === pair[0])
    );
    if (!isDupe) matchupPairs.push(pair);
  }
  ...
  const topBlueBlood = blueBlood[0] as TeamProfile | undefined;
  ...
  if (cinderella && topBlueBlood && cinderella.id !== topBlueBlood.id) {
    const pair: [string, string] = [cinderella.id, topBlueBlood.id];
    const isDupe = matchupPairs.some(
      (p) => (p[0] === pair[0] && p[1] === pair[1]) || (p[0] === pair[1] && p[1] === pair[0])
    );
    if (!isDupe) matchupPairs.push(pair);
  ...
  const predictions = await Promise.all(
    matchupPairs.slice(0, 5).map(([aId, bId]) => predictMatchup(aId, bId))
  );
